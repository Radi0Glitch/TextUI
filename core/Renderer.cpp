...
void Renderer::drawChar(int x, int y, const char* str, const ColorStyle& style) {
    Platform::moveCursor(x, y);
    applyColor(style);
    std::cout << str;
    resetColor();
...
void Renderer::drawLineHorizontal(int x, int y, int length, const char* ch, const ColorStyle& style) {
    Platform::moveCursor(x, y);
    applyColor(style);
...
void Renderer::drawLineVertical(int x, int y, int length, const char* ch, const ColorStyle& style) {
    applyColor(style);
    for (int i = 0; i < length; i++) {
        drawChar(x, y + i, ch, ColorStyle());
    }
    resetColor();
}
